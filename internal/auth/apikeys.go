...
func (s *Service) CreateAPIKey(ctx context.Context, userID, name string, opts ...CreateAPIKeyOptions) (string, *APIKey, error) {
	...
	err := s.pool.QueryRow(ctx,
		`INSERT INTO _ayb_api_keys (user_id, name, key_hash, key_prefix, scope, allowed_tables)
		 VALUES ($1, $2, $3, $4, $5, $6)
		 RETURNING id, user_id, name, key_prefix, scope, allowed_tables, last_used_at, expires_at, created_at, revoked_at`,
		userID, name, hash, prefix, scope, allowedTables,
	...
	s.logger.Info("api key created", "key_id", key.ID, "user_id", userID, "name", name, "scope", scope)
...
func (s *Service) ListAPIKeys(ctx context.Context, userID string) ([]APIKey, error) {
	rows, err := s.pool.Query(ctx,
		`SELECT id, user_id, name, key_prefix, scope, allowed_tables, last_used_at, expires_at, created_at, revoked_at
...
func (s *Service) ValidateAPIKey(ctx context.Context, plaintext string) (*Claims, error) {
	...
	var keyID string
	err := s.pool.QueryRow(ctx,
		`SELECT k.id, k.user_id, k.revoked_at, k.expires_at, k.scope, k.allowed_tables, u.email
		 FROM _ayb_api_keys k
		 JOIN _ayb_users u ON u.id = k.user_id
		 WHERE k.key_hash = $1`,
		hash,
	).Scan(&keyID, &userID, &revokedAt, &expiresAt, &scope, &allowedTables, &email)
	if err != nil {
