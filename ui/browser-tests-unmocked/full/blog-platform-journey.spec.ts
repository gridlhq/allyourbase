 ...
 *
 * UI-ONLY: All interactions through admin dashboard (no direct API calls)
 */
...
test.describe("Blog Platform Journey (Full E2E)", () => {
  test("build blog backend: schema, data, relationships", async ({ page }) => {
    ...
    // Wait for UI to fully load before checking for empty state
    ...
    await sqlInput.fill(authorsSQL);
    await page.getByRole("button", { name: /run|execute/i }).click();

    // Wait for query completion indicator
    ...
    await sqlInput.fill(commentsSQL);
    await page.getByRole("button", { name: /run|execute/i }).click();
    await page.waitForTimeout(500);
    ...
    // All three tables should now be visible in sidebar (reuse sidebar from above)
    ...
    // Status column is type TEXT → RecordForm renders <textarea>, not <input>
    await page.locator('textarea[name="status"], input[name="status"]').or(
      page.locator('label:has-text("status")').locator('..').locator('input,textarea')
    ...
    await page.locator('input[name="title"], textarea[name="title"]').or(
      page.locator('label:has-text("title")').locator('..').locator('input,textarea')
    ).first().fill("Why I Love PostgreSQL");
    await page.locator('textarea[name="content"], input[name="content"]').or(
      page.locator('label:has-text("content")').locator('..').locator('input,textarea')
    ).first().fill("PostgreSQL has been my database of choice for years.");
    // Status column is type TEXT → RecordForm renders <textarea>, not <input>
    await page.locator('textarea[name="status"], input[name="status"]').or(
      page.locator('label:has-text("status")').locator('..').locator('input,textarea')
    ).first().fill("draft");
    ...
    // Verify all posts are still visible after sort
    await expect(page.getByText("Getting Started with AYB")).toBeVisible();
    await expect(page.getByText("Advanced PostgreSQL Tips")).toBeVisible();
    await expect(page.getByText("Why I Love PostgreSQL")).toBeVisible();
    ...
    // Should show FK reference to authors table (multiple elements match — use .first())
    await expect(
      page.getByText(/references|foreign key|authors/i).first(),
    ).toBeVisible();
    ...
    await expect(page.getByText(/references|posts/i).first()).toBeVisible();
    ...
    console.log("\n✅ Blog Platform Journey Complete!");
    console.log("   Created: 3 tables with FK relationships");
    console.log("   Inserted: 2 authors, 3 posts");
    console.log("   Verified: Data browser, filtering, sorting, FK relationships, persistence");
  });
});
