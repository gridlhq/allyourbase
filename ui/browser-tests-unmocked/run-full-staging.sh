...
echo "🧪 Running FULL test suite on STAGING..."
echo "=========================================="
...
export $(cat browser-tests-unmocked/config/.env.staging | grep -v "^#" | grep -v "^$" | xargs)
...
if [ $? -ne 0 ]; then
  echo ""
  echo "❌ Smoke tests FAILED - aborting full suite"
  exit 1
fi
...
if [ $RESULT -eq 0 ]; then
  echo ""
  echo "✅ Staging FULL test suite PASSED"
  echo "   Safe to deploy to production"
  exit 0
else
  echo ""
  echo "❌ Staging full tests FAILED"
